...
Inspecting this output may add further insights to understand the output. For instance, one of the similar profiles (the second) has a *non-malignant* phenotype, while all the others are *malignant*. Looking at D, we can see that the second profile is indeed more distant than the others, although they look close, according to cosine similarity.
