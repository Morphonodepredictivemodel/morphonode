...
The `rfc$RFC` object contains a `randomForest` object, corresponding to the classifier we have just built. In addition, the `rfc$performance` object  includes the performance indices generated by the application of the `rfc$RFC` classifier over the `x$validation.set`.
